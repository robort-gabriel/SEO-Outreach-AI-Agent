...
async def extract_webpage_content(url: str) -> str:
    ...
    try:
        ...
        # Extract links (internal and external) with anchor text analysis
        links = []
        internal_pages = []  # Track internal pages for multi-page analysis
        for link in soup.find_all("a", href=True):
            ...
            is_internal = parsed_url.netloc in full_href or href.startswith("/")
            ...
            # Collect internal pages for multi-page analysis
            if is_internal and full_href not in internal_pages:
                ...
                if not any(pattern in full_href.lower() for pattern in skip_patterns):
                    internal_pages.append(full_href)
        ...
        # Extract URL structure information
        url_path = parsed_url.path
        url_slug = url_path.strip("/").split("/")[-1] if url_path else ""

        # Extract first paragraph for keyword analysis
        first_paragraph = ""
        if main_content:
            first_p = main_content.find("p")
            if first_p:
                first_paragraph = first_p.get_text().strip()[:200]
        else:
            first_p = soup.find("p")
            if first_p:
                first_paragraph = first_p.get_text().strip()[:200]

        # Extract all text for keyword density analysis
        ...
        # Extract internal page links (key pages)
        key_pages = []
        for link in links[:50]:  # Limit to first 50 links
            if link["is_internal"]:
                href = link["href"]
                # Identify key pages by common patterns
                if any(
        ...
        result = {
            ...
            "all_text": all_text[:5000],  # First 5000 chars for keyword analysis
            ...
            "links": {
                ...
                "internal_pages": internal_pages[:20],  # Top 20 internal pages
                "key_pages": key_pages,
            },
...
class LLMService:
    ...
    async def analyze_seo(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            template = """You are an expert SEO auditor. Analyze the following webpage data and provide a comprehensive SEO audit focusing on KEYWORD OPTIMIZATION and VISIBILITY FACTORS.
...
- Internal Pages Found: {internal_pages_count}
- Key Pages: {key_pages}
- Has Canonical URL: {has_canonical}
- Robots Meta: {robots_meta}
- Has Viewport (Mobile): {has_viewport}
- Structured Data: {has_structured_data}

CRITICAL ANALYSIS REQUIRED:
1. **KEYWORD PLACEMENT**: Analyze if primary keywords appear in:
...
2. **KEYWORD DENSITY**: Estimate keyword density (should be 1-2%, not stuffed)

3. **KEYWORD OPPORTUNITIES**: Identify missing keyword opportunities based on:
   - Business type and location
   - Content gaps
   - Long-tail keyword potential

4. **MULTI-PAGE ISSUES**: Note if this is homepage and identify:
   - Missing internal links to key pages
   - Site structure issues
   - Orphaned pages (if detectable)
...
{{
    ...
    "keyword_analysis": {{
        ...
        "keyword_density": "Estimated percentage (1-2% is optimal)",
        "keyword_opportunities": ["List missing keyword opportunities"],
        "issues": ["Keyword-related issues"],
        "suggestions": ["Keyword optimization suggestions"]
    }},
    "site_structure": {{
        "score": 80,
        "internal_linking": "Analysis of internal link structure",
        "key_pages_found": ["List key pages discovered"],
        "issues": ["Site structure issues"],
        "suggestions": ["Site structure improvements"]
    }},
...
```"""
            ...
            url_slug = content.get("url_slug", "")
            internal_pages = content.get("links", {}).get("internal_pages", [])
            key_pages = content.get("links", {}).get("key_pages", [])
            key_pages_str = (
                ", ".join([p.get("url", "") for p in key_pages[:5]]) or "None found"
            )
            ...
            output_text = await chain.ainvoke(
                {
                    "url": content.get("url", ""),
                    "url_slug": url_slug,
                    "is_https": "Yes" if content.get("is_https") else "No",
                    ...
                    "internal_pages_count": len(internal_pages),
                    "key_pages": key_pages_str,
                    "has_canonical": "Yes" if content.get("canonical_url") else "No",
                    "robots_meta": content.get("robots_meta", "Not set"),
                    "has_viewport": "Yes" if content.get("has_viewport") else "No",
                    "has_structured_data": (
