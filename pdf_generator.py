...
class PDFGenerator:
    ...
    def generate_seo_audit_pdf(
        ...
        """Generate a professional PDF from SEO audit data."""
        # Try to import the markdown generation function, fallback to inline generation
        try:
            from website_audit_agent import generate_audit_report_markdown

            # Generate markdown content using the existing function
            report_markdown = generate_audit_report_markdown(
                url=url,
                extracted_content=extracted_content,
                seo_audit=seo_audit,
                recommendations={},  # We don't have recommendations in outreach workflow
                overall_score=overall_score,
                speed_audit=speed_audit,
            )
        except ImportError:
            # Fallback: generate basic markdown if import fails
            report_markdown = self._generate_basic_audit_markdown(
                url=url,
                extracted_content=extracted_content,
                seo_audit=seo_audit,
                overall_score=overall_score,
        ...
        content_html = self._convert_markdown_to_html(report_markdown)
        ...
        template_data = {
            "title": f"SEO Audit Report - {company_name}",
            "description": f"Comprehensive SEO analysis for {company_name} ({url})",
            "content": content_html,
            "keywords": [],
    ...
    def _generate_basic_audit_markdown(
        self,
        url: str,
        extracted_content: Dict[str, Any],
        seo_audit: Dict[str, Any],
        ...
        """Generate basic markdown for SEO audit report (fallback method)."""
        markdown_content = []
